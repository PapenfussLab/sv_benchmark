...
ScoreVariantsFromTruthVCF <- function(callgr, truthgr, includeFiltered=FALSE, maxgap, ignore.strand, sizemargin=0.25,
	...
	callwidth <- .distance(callgr, partner(callgr))
	truthwidth <- .distance(truthgr, partner(truthgr))
	callsize <- callwidth + callgr$insLen
	truthsize <- truthwidth + truthgr$insLen
	...
	hits <- hits[hits$sizeerror - 2 < sizemargin * truthsize$max[hits$subjectHits],]
	# further restrict breakpoint positions for small events
	allowablePositionError <- (pmin(maxgap, truthsize$max / 2) + 2)[hits$subjectHits]
	hits$localbperror <- .distance(callgr[hits$queryHits], truthgr[hits$subjectHits])$min
	hits$remotebperror <- .distance(partner(callgr)[hits$queryHits], partner(truthgr)[hits$subjectHits])$min
