...
truth_hits_df <-
    ...
    select(vcfId, caller_hits_ex_truth, 
           starts_with("fId"), -starts_with("fId0000000000000"))
```
...
truth_hits_df_long <-
    truth_hits_df %>%
    gather(key = "CallSet_Id_subject", value = "qual", starts_with("fId"))
```
...
truth_hits_df_long %>% 
    ...
    left_join(truth_hit_count_df) %>%
    ggplot(aes(
        x = caller_name_lookup[
            str_replace(CallSet_Id_subject, "fId", "")])) +
    scale_y_continuous(expand = c(0,0)) +
    geom_bar(aes(fill = factor(caller_hits_ex_truth)), color = "black") +
    scale_fill_brewer(name = "# callers\nsharing") +
    xlab("caller") +
    coord_flip() +
    ggtitle("Sharing and distribution of true calls")
