...
```{r}
ci_plot <- function(test_id, test_df) {
    ci_ggplot <-
        test_df %>%
        ggplot() +
    	aes(x = logqmean,
...
flipped_hist_plot <- function(test_df) {
    hist_ggplot <- 
        test_df %>%
        ggplot() +
        aes(x = logqmean, y = log10(n)) +
...
stacked_precision_plot <- function(test_id, df) {
    test_df <- df[Id == test_id]
    
    hist_grob <-
        flipped_hist_plot(test_df) %>%
        ggplotGrob(hist_plot)

    ci_grob <-
        ci_plot(test_id, test_df) %>%
        ggplotGrob(ci_plot)
    ...
    combined_grob$heights[panels] <- 
        combined_grob$heights[panels] * c(3,1)
}
